...
class AddressB4a {
  Future<QueryBuilder<ParseObject>> getQueryAll(
    ...
    query.whereEqualTo(AddressEntity.isDeleted, false);
    query.includeObject(['seller', 'region', 'region.seller']);
    return query;
  }
  ...
  Future<List<AddressModel>> list(
    ...
    try {
      ...
      List<AddressModel> listTemp = <AddressModel>[];
      if (parseResponse.success && parseResponse.results != null) {
        for (var element in parseResponse.results!) {
          listTemp.add(AddressEntity().toModel(element));
        }
        return listTemp;
      } else {
        return [];
      ...
      throw B4aException(
        errorTranslated,
        where: 'AddressRepositoryB4a.list',
        originalError:
  ...
  Future<String> update(AddressModel model) async {
    final parseObject = await AddressEntity().toParse(model);
    ParseResponse? parseResponse;
    try {
      ...
      throw B4aException(
        errorTranslated,
        where: 'AddressRepositoryB4a.update',
        originalError:
            '${parseResponse.error!.code} -${parseResponse.error!.message}',
      );
    }
  ...
  Future<bool> delete(String modelId) async {
    ...
    parseObject.set(AddressEntity.isDeleted, true);
    ParseResponse? parseResponse;
